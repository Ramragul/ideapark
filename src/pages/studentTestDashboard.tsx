...
// // Version 3 : Enhancement to version 2 with purple theme support  - Working Version
...
// Version 4 : 3 is the working version, commented for build modifications
...
export const StudentTestDashboard: React.FC = () => {
  ...
  // const testResponse = data as TestResponse | undefined;

  // const { activeTests = [], expiredTests = [] } = testResponse|| {};


  const isTestResponse = (data: unknown): data is TestResponse => {
    return (
      data !== null &&
      typeof data === 'object' &&
      'activeTests' in data &&
      'expiredTests' in data
    );
  };
  
  // Explicitly type activeTests and expiredTests
  let activeTests: Test[] = [];
  let expiredTests: Test[] = [];
  
  if (isTestResponse(data)) {
    activeTests = data.activeTests;
    expiredTests = data.expiredTests;
    // Now you can safely use activeTests and expiredTests
  } else {
