...
// Version 8 : Clone of version 6, Build Version
...
interface StatsResponse {
  assignedCandidates: CandidateStats[];
  attendedCandidates: CandidateStats[];
  notAttendedCandidates: CandidateStats[];
}
...
export const TestStatsPage = () => {
  ...
  // useEffect(() => {
  //   if (data) {
  //     // Example of a runtime check (optional but safer)
  ...
  // function isStatsResponse(data: any): data is StatsResponse {
  //   return (
  //     Array.isArray(data?.assignedCandidates) &&
  //     Array.isArray(data?.attendedCandidates) &&
  //     Array.isArray(data?.notAttendedCandidates)
  //   );
  // }
  ...
  const { data :testData  } = useGetData(
    authState.userId,
    `/api/ip/tests?created_by=${authState.userId}`
  ) as GetDataResponse<Test[]>;
                      ...
                      <Tbody>
                        {statsData.attendedCandidates.map((candidate, index) => (
                          <Tr key={candidate.candidateId}>
                            ...
                            <Td>
                              {((candidate.marksScored ?? 0) / (candidate.totalMarks ?? 1) * 100).toFixed(2)}%
                            </Td>
                          </Tr>
                        ))}
                      </Tbody>
